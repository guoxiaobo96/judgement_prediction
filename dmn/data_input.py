...
def get_data_raw(index, test_id):
    data_map = {
    ...
    if (test_id == ""):
        test_id = index
    data_name = data_map[index]
    data_test_name = data_map[test_id]
    data_train_raw = init_data(os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'data/en-10k/%s_train.txt' % data_name))
    data_test_raw = init_data(os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'data/en-10k/%s_test.txt' % data_test_name))
    return data_train_raw, data_test_raw
...
def load_data(config, split_sentences=False):
    ...
    data_train_raw, data_test_raw = get_data_raw(
        config.data_id, config.data_test_id)
    ...
    train_data = process_input(data_train_raw, config.floatX,
                               word2vec, vocab, ivocab, config.embed_size, split_sentences)
    print('==> get test inputs')
    test_data = process_input(data_test_raw, config.floatX,
                              word2vec, vocab, ivocab, config.embed_size, split_sentences)
    ...
    if config.train_mode:
        train = questions[:config.num_train], inputs[:config.num_train], q_lens[:config.num_train], input_lens[:
                                                                                                               config.num_train], input_masks[:config.num_train], answers[:config.num_train]
